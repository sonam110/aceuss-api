...
class CategoryMasterController extends Controller
{
    ...
    public function store(Request $request){
        ...
        try {
		    ...
		    [
            'category_type_id.required' => getLangByLabelGroups('CategoryMaster','message_category_type_id'),
            'name.required' => getLangByLabelGroups('CategoryMaster','message_name'),
            ]);
	        if ($validator->fails()) {
            	return prepareResult(false,$validator->errors()->first(),[], config('httpcodes.bad_request')); 
        	}
        	...
        	if($checkAlready) {
              	return prepareResult(false,getLangByLabelGroups('CategoryMaster','message_name_already_exists'),[], config('httpcodes.bad_request')); 
        	}
    ...
    public function update(Request $request,$id){
        ...
        try {
            ...
            $data = CategoryMaster::where('id', $categoryMaster->id)
                    ->with('Parent:id,name','CategoryType:id,name')
                    ->first();
	        return prepareResult(true,getLangByLabelGroups('CategoryMaster','message_update') ,$data, config('httpcodes.success'));
