...
class ScheduleController extends Controller
{
    ...
    public function store(Request $request){
        ...
        try {
            ...
            foreach($request->shift_dates as $key=>$shift_date)
            {
                ...
                $schedule->date = ($request->date)? $request->date :date('Y-m-d');
                $schedule->time = ($request->time)? $request->time :date('h:i');
                $schedule->description = $request->description;
                $schedule->entry_mode =  (!empty($request->entry_mode)) ? $request->entry_mode :'Web';
                $schedule->is_signed = ($request->is_signed)? $request->is_signed :0;
                $schedule->is_secret = ($request->is_secret)? $request->is_secret :0;
                $schedule->is_active = ($request->is_active)? $request->is_active :0;
                $schedule->edit_date = date('Y-m-d H:i:s');
                $schedule->save();
            }
    ...
    public function update(Request $request,$id){
        ...
        try {
            ...
            [   
                'category_id' =>  getLangByLabelGroups('Schedule','message_category_id'), 
                'subcategory_id' =>  getLangByLabelGroups('Schedule','message_subcategory_id'), 
                // 'description' =>  getLangByLabelGroups('Schedule','message_description'), 
            ]);
	        if ($validator->fails()) {
            	return prepareResult(false,$validator->errors()->first(),[], config('httpcodes.bad_request')); 
        	}
            ...
            $schedule->branch_id = getBranchId();
		 	$schedule->patient_id = $request->patient_id;
		 	$schedule->emp_id = $user->id;
		 	$schedule->category_id = $request->category_id;
		 	$schedule->subcategory_id = $request->subcategory_id;
		 	$schedule->description = $request->description;
            $schedule->date = ($request->date)? $request->date :date('Y-m-d');
            $schedule->time = ($request->time)? $request->time :date('h:i');
		 	$schedule->edited_by = $user->id;
		 	$schedule->reason_for_editing = $request->reason_for_editing;
            $schedule->edit_date = date('Y-m-d H:i:s');
            $schedule->entry_mode =  (!empty($request->entry_mode)) ? $request->entry_mode :'Web';
            $schedule->is_signed = ($request->is_signed)? $request->is_signed :0;
            $schedule->is_secret = ($request->is_secret)? $request->is_secret :0;
